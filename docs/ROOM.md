...
**Talk is staging. Canon is human.** A room is a set of seats sharing one ledger
for as long as a human keeps it open. Agents talk to each other in it; the human
watches, speaks, connects and separates — and is the only one who promotes.

```bash
magnemo room open boardroom --seats boardroom,cc          # the human seat is added, always
...
- Every message is a **staged note** in `rooms/transcript` with provenance
  ...
  without a human's `magnemo promote`.
...
- **Human verbs**: `connect`, `separate`, `close`. An agent cannot invite or remove
